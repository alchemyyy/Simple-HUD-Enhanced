...
public class EquipmentCache {
    ...
    public void updateCache(LocalPlayer player) {
    ...
    private void createEquipment(LocalPlayer player) {
        ...
        equipmentInfo = Services.PLATFORM.getEquipmentInfo(player, config);
        ...
        if (!config.equipmentStatus.slots.Head) {
            equipmentInfo.removeIf(equipment -> equipment.getItem().equals(this.player.getInventory().getArmor(3)));
        }
        if (!config.equipmentStatus.slots.Body) {
            equipmentInfo.removeIf(equipment -> equipment.getItem().equals(this.player.getInventory().getArmor(2)));
        }
        if (!config.equipmentStatus.slots.Legs) {
            equipmentInfo.removeIf(equipment -> equipment.getItem().equals(this.player.getInventory().getArmor(1)));
        }
        if (!config.equipmentStatus.slots.Boots) {
            equipmentInfo.removeIf(equipment -> equipment.getItem().equals(this.player.getInventory().getArmor(0)));
        }
    ...
    private void getDurability() {
        ...
        for (EquipmentInfoStack index : equipmentInfo) {
            ...
            if (item.getMaxDamage() != 0) {
                ...
                if (config.equipmentStatus.showCount && this.player.getInventory().countItem(item.getItem()) > 1) {
                    // Check if player is holding all the item
                    if (this.player.getInventory().countItem(item.getItem()) == item.getCount()) {
                        index.setText(String.valueOf(item.getCount()));
                    } else {
                        index.setText((item.getCount() + " (" + this.player.getInventory().countItem(item.getItem()) + ")"));
                    }
                } else {
                    index.setText("");
                }
    ...
    private boolean isItemDurabilityDisabled(ItemStack item) {
        ...
        } else if (item.equals(this.player.getInventory().getArmor(0))) {
            return !config.equipmentStatus.Durability.slots.Boots;
        } else if (item.equals(this.player.getInventory().getArmor(1))) {
            return !config.equipmentStatus.Durability.slots.Legs;
        } else if (item.equals(this.player.getInventory().getArmor(2))) {
            return !config.equipmentStatus.Durability.slots.Body;
        } else if (item.equals(this.player.getInventory().getArmor(3))) {
